      ...
      <main className="mx-auto max-w-6xl px-4 py-10 sm:px-6">
        <header className="mb-8 flex items-center gap-4">
          ...
          <div className="min-w-0">
            <h1 className="font-sora text-2xl font-semibold tracking-tight">{sellerName}</h1>
            <p className="text-sm text-muted-foreground">
              {profile.tagline ?? `${items.length} product${items.length === 1 ? "" : "s"} available`}
            </p>
          </div>
          <Link
            href="/"
        ...
        {items.length === 0 ? (
          <p className="py-20 text-center text-muted-foreground">No products live yet. Check back soon.</p>
        ) : (
          <StoreCatalog items={items} categories={categories} base="/store" />
        )}
      </main>
      <CartDrawer />
    </CartProvider>
